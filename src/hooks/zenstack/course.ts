...
export function useCheckCourse<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; nativeLanguageId?: string; targetLanguageId?: string; outcome?: string; level?: string }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
