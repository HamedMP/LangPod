...
export function useCheckLesson<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; title?: string; type?: string; scenarioContext?: string; audioUrl?: string; audioUrls?: string; courseId?: string; progress?: ProgressStatus }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
